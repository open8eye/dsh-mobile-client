...
取决于手机此刻在哪，而这件事 App 存不下来——存下来的答案，正好会在它错的时候被用上。

所以设备存 `baseUrl` + `altBaseUrls`，连接时 `AddressProbe` **同时**探测所有候选，
第一个回应的胜出：

| 设计点 | 原因 |
|---|---|
| 只有一个候选时**不探测** | 探测只会给一次整页加载再加一个 RTT，多数用户只有一个地址 |
| 探测并行、取先回应者 | 两个都通时，胜出的是**更快**的那个，正好是想要的那个 |
| 只等响应头，不读 body | DSH 的根路径要么给应用、要么给登录页，两者都说明地址通；正文归随后的会话 |
| 全部失败返回 `null` | 调用方回退到主地址，让 WebView 显示它自己那个信息量大得多的失败页 |
| 结果**不持久化** | 存下来的是「上次在哪」，而需要的答案是「现在在哪」 |

探测走 Dart 的 `HttpClient`，和 WebView 走同一套系统网络栈，所以 Tailscale 这类
网络层隧道对两者是等价的——探测能通，WebView 就能通。
